...
import { ArchivingService } from 'src/app/core/service/archiving.service';

@Component({
  selector: 'app-archiving-view',
  templateUrl: './archiving-view.component.html',
  styleUrls: ['./archiving-view.component.scss']
})
export class ArchivingViewComponent implements OnInit {
  ...
  constructor(private ActivatedRouter: ActivatedRoute, private route: Router, private service: ArchivingService, private CommonService: CommonService) { }
  ...
  ngOnInit() {
    ...
    this.filter_section_data.date_range = { start : this.filter_section_data.from_date, end : this.filter_section_data.to_date };
  ...
  selectButtonTextIconChange(flag){
    if(flag == 1){ // show selected
      ...
      this.selectbuttonText = this.language.clear_filter;
        ...
        if (data != null) {
          if (data.length > 0) {
            ...
            if(data.length > 0){
              ...
              this.CommonService.show_notification(this.language.no_records_found_in_range, 'error');
            }
          } else {
      ...
      data => {
        ...
        if (data != null) {
          if (data.length > 0) {
            ...
            // here goes the result processing 
            this.CommonService.show_notification(this.language.cnf_deleted_successfully, 'success');
            this.clear_filter_results(0);
            this.showLookupLoader = false;
